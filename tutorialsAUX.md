...
4- Start the VM and go to the Ubuntu installation process.

5- Run the following commands:
  - sudo apt dist-upgrade
  - sudo apt update

6- Delete the SSH keys provided by default:
...
2- Go to the Cloud-Init tab and configure it as you like (Do not forget to click on Regenerage Image when you are done).

3- Start the VM and run the following commands:
  - sudo cloud-init clean --machine-id
  - sudo start cloud-init
  - sudo reboot

4- Now, you can do SSH to the VM. Finally, install QEMU Guest Agent (ensure that it is enabled in the Options tab):
